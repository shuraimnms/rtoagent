 ...
 * @access  Private (Admin)
 */
exports.verifyCashfreeConfig = async (req, res) => {
  try {
    ...
    // Try to get a test order status (this will verify API credentials)
    const testResult = {
      configured: true,
      baseUrl: cashfreeService.baseUrl,
      appId: cashfreeService.appId ? 'Configured' : 'Not configured',
      secretKey: cashfreeService.secretKey ? 'Configured' : 'Not configured',
      callbackUrl: cashfreeService.callbackUrl,
      isProduction: cashfreeService.isProduction
    };

    res.json({
      success: true,
      message: 'Cashfree configuration status retrieved.',
      data: testResult
    });
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to verify Cashfree configuration',
      error: error.message
    });
  }
};
 ...
 * @access  Private (Admin)
 */
exports.getAgentRoleByEmail = async (req, res) => {
  try {
    const { email } = req.params;
    const agent = await Agent.findOne({ email }).select('role');
    ...
    console.error('Get agent role by email error:', error);
    res.status(400).json({
      success: false,
      message: error.message
    });

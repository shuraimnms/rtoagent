...
exports.createTopupOrder = async (req, res) => {
  try {
    ...
    if (selectedGateway === 'razorpay') {
      ...
      // ✅ Corrected endpoint: /session (not /sessions)
      const sessionResponse = await axios.post(
        cashfreeSettings.cashfree.isProduction
          ? `https://api.cashfree.com/pg/orders/${orderId}/session`
          : `https://sandbox.cashfree.com/pg/orders/${orderId}/session`,
        {
          payment_method: {
            card: true,
            netbanking: true,
    ...
    console.error('Create topup order error:', error.response?.data || error.message);
    res.status(400).json({
      success: false,
      message: error.message,
    });
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        summary: {
          total_spent: monthlySpending.reduce((sum, m) => sum + m.total_spent, 0),
          total_topups: topupHistory.reduce((sum, t) => sum + t.amount, 0),
          average_monthly_spend:
            monthlySpending.length > 0
              ? monthlySpending.reduce((sum, m) => sum + m.total_spent, 0) / monthlySpending.length
              : 0,
        },
      },
    });
